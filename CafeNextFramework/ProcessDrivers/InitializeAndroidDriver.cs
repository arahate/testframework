...
{
    class InitializeAndroidDriver : IInitializeWebDriver
    {
        public List<IWebDriver> Get()
        {
            ...
            foreach (PlatformConfiguration platform in platforms)
            {
                if (!string.IsNullOrEmpty(platform.AutomationPlatformName) &&
                    platform.AutomationPlatformName.Equals(Constant.ANDROID_DRIVER,StringComparison.OrdinalIgnoreCase))
                {
                    try
                    {
                    ...
                    {
                        Console.WriteLine("Not Able to launch the Android Driver" + e.Message);
                    }
                }
            }
            return drivers;
